...
- 출처 : [https://www.acmicpc.net/problem/1330](https://www.acmicpc.net/problem/13300)
...
- 첫 번째 줄에는 수학여행에 참가하는 학생 수를 나타내는 정수 N(1 ≤ N ≤ 1,000)과 
              한 방에 배정할 수 있는 최대 인원 수 K(1 < K ≤ 1,000)가 
  공백으로 분리되어 주어진다. 
...
public class Main {
	...
	public static void main(String[] args) throws IOException {
        ...
        // 1 ~ 6학년 , 남 & 여 구분
        int[][] studentInfo = new int[2][7];    //[성별][학년 0~6], 
        
		for (int i = 0; i < n; i++) {  // 총 인원수 만큼 배열 값 초기화
            st = new StringTokenizer(bf.readLine());
			int sex = Integer.parseInt(st.nextToken());	// 성별
			int grade = Integer.parseInt(st.nextToken());	// 학년
		
            if(sex == 0) {
                studentInfo[0][grade] ++;   // [1~6][여자]
            } else {
                studentInfo[1][grade] ++;   // [1~6][남자]
            }
        }
        
        for (int i = 0; i < studentInfo.length; i++) {  
            for (int j = 1; j < studentInfo[i].length; j++) {  // 0학년없음 1부터~
                if (studentInfo[i][j] == 0) {
                    continue;
                }

                // get room
                room += studentInfo[i][j] / k;
                if (studentInfo[i][j] % k != 0) {
                    room++;
                }
            }
        }
